...
to invert a binary tree, can simply switch the left and right nodes.

1. decide on the return value
    for the return value, his questions asks for an inverted version of the original tree, so each step we return the 
    inverted version of the current subtree
