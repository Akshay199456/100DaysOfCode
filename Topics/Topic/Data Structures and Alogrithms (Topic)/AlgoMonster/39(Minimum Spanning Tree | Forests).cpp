...
/*
-------------------------    Notes
main idea is to run kruska's algorithm with some tweaked aspects. instead of terminating when -1 nodes have been mered, we are notified that it may not be oossible to merge every node. therefore, we simply just let the algorithm run until every ede is checked. one can also run prim's but make sure that every single node
is calculated. every connected componmnet needs to have prim's algorithm run on it.
