...
/*
-------------------------    Notes
the systematic approach is to check the sum of each subarray with size k and find the largest one. we start frm the leftmost
window of size k in the array, and we can keep track of a window_sum variable that stores the sum of the current window.
when we shift the window to the right, we add a new element to the right of the window and remove the leftmost element from the window.
to obtain the window_sum of the new window, we can add the value of the new elmeent and subtract the value we removed on the left. then, for each window, we wish 
to check whether the window_sum is at least largest where largest is the current largest sum among all processed windows.
