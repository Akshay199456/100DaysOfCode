...
/*
-------------------------    Notes
problem follows the top k numbers pattern and is quite similar to rearrage string problem. only difference is that in the 'rearrange string', the same chracters need not be adjacent i.e they should be at least 2 distance aprt (in other words, there should be at least one character betweeen the two same characters) while in the 
current prb;em,. the same characters should be 'K' distance appart.

following similar approach, since we were inserting a character back in the hea[ n the next iteration, in this proble,m, we will
re-insert the chracter after k iterations. we can keep track of the previous characters ni a quueeu to insert them back in the heap after k iterations
